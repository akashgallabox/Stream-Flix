const { getDb, checkAuth, json } = require("../_utils");
...
module.exports = async function handler(req, res) {
  ...
  const newVideo = {
    title, description: description || "External stream link",
    duration: Number(duration) || 300, durationStr: durationStr || "5m 00s",
    size: 0, url, isExternal: true,
    thumbnailUrl: "https://images.unsplash.com/photo-1485846234645-a62644f84728?q=80&w=800&auto=format&fit=crop",
    category: category || "Other", uploadedAt: new Date().toISOString(),
    views: 0, rating: `${90 + Math.floor(Math.random() * 10)}% Match`,
    year: year || "2026", maturityRating: maturityRating || "PG-13",
  };
  ...
  try {
    ...
    console.error("[POST /api/videos/external]", err?.message);
